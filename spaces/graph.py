    ...
    def sample(
        ...
        if num_edges is None:
            ...
            if edge_space_mask is not None:
                edge_space_mask = tuple(
                    edge_space_mask for _ in range(num_edges))
        else:
        ...
        sampled_node_space = self._generate_sample_space(self.node_space,
                                                         num_nodes)
        sampled_edge_space = self._generate_sample_space(self.edge_space,
                                                         num_edges)
                    ...
                    if isinstance(x.edges, np.ndarray) and isinstance(
                        ...
                        if self.edge_space is not None:
                            if all(edge in self.edge_space for edge in x.edges):
                                if np.issubdtype(x.edge_links.dtype,
                                                 np.integer):
