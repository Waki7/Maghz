...
class Sequence(Space[Tuple]):
    ...
    def sample(
            self,
            mask: Optional[
                Tuple[Optional[Union[np.ndarray, int]], Optional[Any]]] = None,
    ) -> Tuple[Any]:
    ...
    def to_jsonable(self, sample_n: list) -> list:
        ...
        return [self.feature_space.to_jsonable(list(sample)) for sample in
                sample_n]

    def from_jsonable(self, sample_n: List[List[Any]]) -> list:
        """Convert a JSONable data type to a batch of samples from this space."""
        return [tuple(self.feature_space.from_jsonable(sample)) for sample in
                sample_n]

    def __eq__(self, other) -> bool:
        """Check whether ``other`` is equivalent to this instance."""
        return isinstance(other,
                          Sequence) and self.feature_space == other.feature_space
