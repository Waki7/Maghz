...
from typing import Dict, List, Optional, Sequence, SupportsFloat, Tuple, Type, \
    Union

import numpy as np

from .space import Space
from .gym_logger import warn
...
def is_float_integer(var) -> bool:
    ...
    return np.issubdtype(type(var), np.integer) or np.issubdtype(type(var),
                                                                 np.floating)
    ...
    * Independent bound for each dimension::
    ...
    def __init__(
        ...
        if shape is not None:
            ...
            shape = tuple(
                int(dim) for dim in shape)  # This changes any np types to int
        ...
        _low = np.full(shape, low, dtype=float) if is_float_integer(
            low) else low
        self.bounded_below = -np.inf < _low
        _high = np.full(shape, high, dtype=float) if is_float_integer(
            high) else high
        ...
        if min(low_precision, high_precision) > dtype_precision:  # type: ignore
            warn(
                f"Box bound precision lowered by casting to {self.dtype}")
    ...
    def contains(self, x) -> bool:
        ...
        if not isinstance(x, np.ndarray):
            warn("Casting input x to numpy array.")
            try:
    ...
    def from_jsonable(self, sample_n: Sequence[Union[float, int]]) -> List[
        np.ndarray]:
...
def _broadcast(
    ...
    if is_float_integer(value):
        value = get_inf(dtype, inf_sign) if np.isinf(
