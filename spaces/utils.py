...
def _flatten_dict(space, x) -> Union[dict, np.ndarray]:
    ...
    return OrderedDict(
        (key, flatten(s, x[key])) for key, s in space.spaces.items())
    ...
    def _graph_unflatten(unflatten_space, unflatten_x):
        ...
        if unflatten_space is not None and unflatten_x is not None:
            if isinstance(unflatten_space, Box):
                ...
                ret = np.zeros(
                    (unflatten_x.shape[0],
                     unflatten_space.n - unflatten_space.start),
                    dtype=unflatten_space.dtype,
                )
                ret[
                    np.arange(unflatten_x.shape[
                                  0]), unflatten_x - unflatten_space.start
                ] = 1
        return ret
...
def _flatten_text(space: Text, x: str) -> np.ndarray:
    arr = np.full(
        shape=(space.max_length,), fill_value=len(space.character_set),
        dtype=np.int32
    )
...
def _unflatten_multidiscrete(space: MultiDiscrete, x: np.ndarray) -> np.ndarray:
    ...
    return np.asarray(indices - offsets[:-1], dtype=space.dtype).reshape(
        space.shape)
...
def _unflatten_tuple(space: Tuple, x: Union[np.ndarray, tuple]) -> tuple:
    ...
    return tuple(
        unflatten(s, flattened) for flattened, s in zip(x, space.spaces))


@unflatten.register(Dict)
def _unflatten_dict(space: Dict, x: Union[np.ndarray, TypingDict]) -> dict:
    if space.is_np_flattenable:
        dims = np.asarray([flatdim(s) for s in space.spaces.values()],
                          dtype=np.int_)
        list_flattened = np.split(x, np.cumsum(dims[:-1]))
        return OrderedDict(
            [
                (key, unflatten(s, flattened))
                for flattened, (key, s) in
                zip(list_flattened, space.spaces.items())
            ]
        )
    assert isinstance(
        x, dict
    ...
    return OrderedDict(
        (key, unflatten(s, x[key])) for key, s in space.spaces.items())
...
def _unflatten_text(space: Text, x: np.ndarray) -> str:
    return "".join(
        [space.character_list[val] for val in x if
         val < len(space.character_set)]
    )
...
def _flatten_space_binary(
        space: Union[Discrete, MultiBinary, MultiDiscrete]) -> Box:
...
def _flatten_space_text(space: Text) -> Box:
    return Box(
        low=0, high=len(space.character_set), shape=(space.max_length,),
        dtype=np.int32
    )
