...
class TestBert(unittest.TestCase):
    ...
    def test_distance_measures_euclidean_nrom_sparseMatch(self):
        class_embeddings: FloatTensorT['NClasses,EmbedLen'] = torch.tensor(
            [[1, 1, 1],
             [1, 2, 3],
             [5, 5, 5]]).float()
        ...
        distance_to_classes: FloatTensorT['NQuery,NClasses'] = (
            DistanceMeasuresPerClass.euclidean_distance(
                class_embeddings=class_embeddings,
                query_embeddings=query_embeddings, normalize=True))
        print(distance_to_classes)

        self.assertTrue(distance_to_classes.shape == (
            query_embeddings.shape[0], class_embeddings.shape[0]))
        self.assertTrue(
            distance_to_classes[0, 0] < distance_to_classes[0, 1])
        self.assertTrue(
            distance_to_classes[0, 0] < distance_to_classes[0, 2])
        self.assertTrue(
            distance_to_classes[1, 2] < distance_to_classes[1, 0])
        self.assertTrue(
            distance_to_classes[1, 2] < distance_to_classes[1, 1])
        probs = torch.softmax(distance_to_classes, dim=-1)
        print(probs)
        print(probs.argmax(dim=-1))
