...
class MobileNetV2(BaseModel):
    ...
    def __init__(self, in_space: sp.Space, cfg={}, **kwargs):
        """
        ...
        input_channel = 32
        last_channel = 1280
        if isinstance(in_space, sp.Tuple):
            assert len(in_space.spaces) == 1
            in_space = in_space.spaces[0]
        assert isinstance(in_space, sp.Box), \
            'only handling unimodal, image observation rn'
        self.in_channels = in_space.shape[-3]
        ...
        super(MobileNetV2, self).__init__(in_space=in_space,
